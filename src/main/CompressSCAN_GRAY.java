...
import ac.*;
import io.WriterGRAY;
import model.BestPathOutput;
import model.Block;
import model.BlockErrorOutput;
import model.BlockGRAY;
import model.Path;
import model.Pixel;
import scanpaths.ConstantsScanGRAY;
import scanpaths.ScanPathsGRAY;

public class CompressSCAN_GRAY {
	...
	public CompressSCAN_GRAY(String pathInputFile, String pathOutFile) throws IOException {

		scannedPixel = new HashMap<String, String>();
		u_pixel = null;
		v_pixel = null;
		matrix = loadImage(ImageIO.read(new File(pathInputFile)));
		blocks = BlockGRAY.getBlocks(matrix, ConstantsScanGRAY.blockSize);
	...
	public void compress() throws IOException {
		...
		for (String scan : scanPathsName)
			encodeScanPaths += encode(scan, ConstantsScanGRAY.blockSize);
		...
		WriterGRAY writer = new WriterGRAY();
	...
	private BestPathOutput BestPath(Block block, Pixel prevLastPixel) {
		...
		ScanPathsGRAY s = new ScanPathsGRAY();
		...
		for (int i = 0; i < k.length; i++) {
			for (t = 0; t < ConstantsScanGRAY.maxDirectionScan; t++) {
		...
		int B = 4;
		if (block.length() > ConstantsScanGRAY.minimumBlockSize)
			B = 5;
	...
	private String encode(String scanpath, int blockSize) {
		...
		if (scan.length() == 2) {
			...
			if (blockSize > ConstantsScanGRAY.minimumBlockSize)
